...
- Excel/CSV 브라우저 분석, 시트·열 선택, 학년도 날짜 계산
- 수정·제외 가능한 미리보기와 기존 학교 일정 중복 경고
- 선택한 항목의 `schoolSchedule` 종일 일정 일괄 등록
- PDF·이미지·한글 문서 인식과 가져오기 일괄 되돌리기는 후속 범위
...
- 중앙 날짜·근무 상태 안내와 월간 통합 캘린더
- 오른쪽 오늘의 업무, 회신 대기, 회의·일정과 운동
- 페이지 이동 없이 기존 생성 패널을 여는 빠른 추가 진입점
- 모바일 하단 내비게이션과 단일 열 카드 흐름
- 보건업무, 학교일정, 제출·마감, 회신 대기, 개인일정과 운동의 시각 구분
...
- 오늘 날짜와 근무 상태 안내, 월간 일정과 우선 업무를 페이지 이동 없이 확인할 수 있다.
...
Phase 4 이후의 repository 확장, 체크리스트, 별도 `daily_focus_assignments`, 운동 기록과 반복 업무는 Phase 3 완료에 포함하지 않는다. 아래 후속 Phase의 업무·일정 항목 중 Phase 3에서 이미 제공한 기본 CRUD·브리핑·월간 표시는 재구현하지 않고 고급 기능만 확장한다.
...
v1.0에서는 미완성 프로젝트 관리 기능을 제공하지 않는다. 프로젝트 영역으로 생성된 기존 Task는 일반 업무로 보존하고, 프로젝트 화면·생성·필터 진입점은 제거한다. 여러 업무를 하나의 목표로 묶는 기능은 실제 사용 피드백으로 필요성이 확인된 뒤 데이터 모델과 사용자 흐름을 새로 검토한다.
...
- [x] 학사일정 스티커팩 28개 registry와 로컬 SVG 추가 (`동아리` 포함)
...
- [x] 1월부터 12월까지 월별 기본 보건업무 제안과 현재 월 강조
- [x] 기존 보건업무 프리셋 registry를 재사용한 Task·Event 생성 폼 자동 입력
- [x] 날짜를 강제 지정하지 않고 사용자 확인 전 저장을 차단하는 연도·월 안내
...
월별 목록의 `보건실 월간·연간 통계`는 기존 Task 생성 폼을 채우는 정적 제안이며 통계 기능 구현으로 보지 않는다.

## 빠른 보건업무 생성 동선 (Phase 8 아님)

- [x] 연간 플래너의 공통 보건업무 preset registry 재사용
- [x] 업무 화면 우선 프리셋 6개와 그룹별 전체 12개 표시
- [x] 모바일 새로 만들기 sheet의 빠른 보건업무와 기존 메뉴 유지
