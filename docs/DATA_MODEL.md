...
- `profiles.role`은 `user`, `admin`, `owner`만 허용하며 신규 사용자는 `user`다.
- `profiles` 수정 정책은 본인의 `display_name`, `avatar_url`만 허용하고 `id`, `email`, `role` 변경은 trigger가 차단한다.
- `notices`는 `notice`, `update`, `maintenance`, `important` 카테고리와 요약, 관리자 작성 공지, UTC 게시 기간을 저장한다.
- `notice_reads`는 `(notice_id, user_id)` 단위의 사용자별 읽음 상태를 저장한다.
- 역할 변경은 앱 UI에서 제공하지 않으며 Supabase 관리 SQL로만 수행한다.
