...
- 원본 파일을 브라우저에서 읽고 시트·열·학년도 옵션을 정규화된 미리보기 모델로 변환한다.
- UI는 Supabase를 직접 호출하지 않고 서버 액션을 통해 중복 확인과 최종 등록을 요청한다.
- `lib/academic-calendar-import/parser.ts`는 React·Supabase·브라우저 API에 의존하지 않는 순수 파서다.
- `lib/academic-calendar-import/repository.ts`가 인증 사용자 확인과 RLS 대상 `events` 일괄 insert를 담당한다.
