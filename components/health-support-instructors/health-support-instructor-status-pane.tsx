...
export function SettlementPanel({ documents, onMonthChange, onPrint, onToggleDetail, printActive, selectedMonth, showWorkDetail }: SettlementPanelProps) {
  ...
  return <section aria-label="Monthly settlement document" className="health-support-dashboard health-support-document health-support-document--work-detail">
    <div className="health-support-dashboard__summary">
      <div><h3 aria-label="Selected month settlement">선택 월 정산</h3><label>정산 월<input aria-label="Settlement month" onChange={(event) => onMonthChange(event.target.value)} type="month" value={selectedMonth} /></label></div>
      <article><h3 aria-label="Work and wage summary">근무·급여 요약</h3><div><span>근무 시간</span><strong>{settlement.hours.toFixed(1)}시간</strong></div><div><span>시간당 단가</span><strong>{won(settlement.hourlyRate)}</strong></div><div><span>인건비</span><strong>{won(settlement.wages)}</strong></div><div><span>월 보험료</span><strong>{won(settlement.insurance)}</strong></div><div><span>정산 총액</span><strong>{won(settlement.total)}</strong></div></article>
    </div>
    <div className="health-support-dashboard__grid">
      <article><h3 aria-label="Limit and budget status">한도·예산 현황</h3><p>예산 집행 {won(documents.budget.spent)} / {won(documents.budget.total)}</p><p>예산 잔액 {won(documents.budget.remaining)}</p>{documents.warnings.length === 0 ? <p>선택한 달의 주간·월간 한도 경고가 없습니다.</p> : <ul>{documents.warnings.map((warning) => <li key={`${warning.kind}-${"isoWeekKey" in warning ? warning.isoWeekKey : warning.month}`}>{settlementWarningLabel(warning)}</li>)}</ul>}</article>
    </div>
    <div className="health-support-document__actions"><Button aria-label="Preview work detail" onClick={onToggleDetail} variant="secondary">근무내역 미리보기</Button><Button aria-label="Print work detail" onClick={onPrint} variant="secondary">근무내역 출력</Button></div>
...
function settlementWarningLabel(warning: HealthSupportSettlementDocuments["warnings"][number]): string {
  const labels = { monthlyLimit: "Monthly limit reached", weeklyLimit: "Weekly limit reached", weeklyWarning: "Weekly warning" } as const;
