...
import { attachPracticalToolAction, createAndAttachPracticalToolAction, deletePracticalScheduleAction, detachPracticalToolAction, linkExistingEventAction, savePracticalScheduleAction, type PracticalScheduleActionState } from "@/app/(app)/practical-schedules/actions";
...
function ScheduleToolsModal({ item, tools, links, practicalToolsError = false, scheduleToolLinksError = false, onClose, returnFocusRef }: { readonly item: PracticalScheduleRow; readonly tools: readonly PracticalTool[]; readonly links: readonly PracticalScheduleTool[]; readonly practicalToolsError?: boolean; readonly scheduleToolLinksError?: boolean; readonly onClose: () => void; readonly returnFocusRef: RefObject<HTMLButtonElement | null> }) {
  const [addOpen, setAddOpen] = useState(false);
  const [tab, setTab] = useState<"public" | "personal" | "direct">("personal");
  const [state, action, pending] = useActionState(createAndAttachPracticalToolAction, idleActionState);
  const linked = useMemo(() => new Set(links.filter((link) => link.schedule_id === item.id).map((link) => link.tool_id)), [item.id, links]);
  const linkedTools = tools.filter((tool) => linked.has(tool.id));
  const candidates = tools.filter((tool) => tool.is_active && !linked.has(tool.id) && (tab === "direct" || tool.scope === tab));
  useEffect(() => { if (state.status === "success") { window.setTimeout(() => { setAddOpen(false); setTab("personal"); }, 0); } }, [state.status]);
  return <ResponsiveDetailPanel onClose={onClose} open panelClassName="practical-schedule-tools-panel" presentation="modal" returnFocusRef={returnFocusRef} title={item.title}>
    <div className="practical-tools-detail">
      <header className="practical-tools-detail__header"><div><h2>관련 도구·링크</h2><p>이 업무에 필요한 도구, 사이트, 문서 링크를 연결해두고 바로 열 수 있어요.</p></div><button className="button button--secondary" onClick={() => setAddOpen((open) => !open)} type="button"><Plus size={16} />도구 추가</button></header>
      {scheduleToolLinksError ? <p className="practical-tools-detail__error" role="alert">도구 목록을 불러오지 못했어요. 다시 시도해 주세요.</p> : linkedTools.length === 0 ? <div className="practical-tools-detail__empty"><strong>아직 연결된 도구·링크가 없어요.</strong><span>자주 사용하는 사이트나 업무 도구를 연결해보세요.</span><button className="button button--secondary" onClick={() => setAddOpen(true)} type="button"><Plus size={15} />도구 추가</button></div> : <div className="practical-tools-detail__groups">{(["public", "personal"] as const).map((scope) => { const scopedTools = linkedTools.filter((tool) => tool.scope === scope); if (!scopedTools.length) return null; return <section key={scope}><h3>{scope === "public" ? "공용 도구" : "내 도구 (나만 볼 수 있어요)"}</h3><div className="practical-tools-detail__grid">{scopedTools.map((tool) => <article className="practical-tool-card" key={tool.id}><div className="practical-tool-card__body"><div className="practical-tool-card__identity"><PracticalToolIcon iconKey={tool.icon_key} /><div><div className="practical-tool-card__name-row"><strong>{tool.name}</strong><span className="practical-tool-scope-badge">{tool.scope === "public" ? "공용" : "내 도구"}</span></div>{tool.description && <p>{tool.description}</p>}<small>{!tool.is_active && "비활성"}</small></div></div></div><div className="practical-tool-card__actions">{tool.url.startsWith("/") ? <Link href={tool.url}>열기 <ExternalLink aria-hidden="true" size={14} /></Link> : <a href={tool.url} rel="noopener noreferrer" target="_blank">열기 <ExternalLink aria-hidden="true" size={14} /></a>}<form action={detachPracticalToolAction}><input name="scheduleId" type="hidden" value={item.id} /><input name="toolId" type="hidden" value={tool.id} /><button type="submit">연결 해제</button></form></div></article>)}</div></section>; })}</div>}
      {addOpen && <section className="practical-tools-detail__add"><div className="practical-tools-detail__add-head"><h3>도구 추가하기</h3><button aria-label="도구 추가 닫기" className="icon-button" onClick={() => setAddOpen(false)} type="button"><X size={16} /></button></div><div className="practical-tools-tabs" role="tablist"><button aria-selected={tab === "public"} className={tab === "public" ? "is-active" : undefined} onClick={() => setTab("public")} role="tab" type="button">공용 도구</button><button aria-selected={tab === "personal"} className={tab === "personal" ? "is-active" : undefined} onClick={() => setTab("personal")} role="tab" type="button">내 도구</button><button aria-selected={tab === "direct"} className={tab === "direct" ? "is-active" : undefined} onClick={() => setTab("direct")} role="tab" type="button">직접 입력</button></div>{tab !== "direct" ? <div className="practical-tools-detail__candidates">{practicalToolsError ? <p className="practical-tools-detail__error" role="alert">도구 목록을 불러오지 못했어요. 다시 시도해 주세요.</p> : candidates.length ? candidates.map((tool) => <form action={attachPracticalToolAction} key={tool.id}><input name="scheduleId" type="hidden" value={item.id} /><input name="toolId" type="hidden" value={tool.id} /><span className="practical-tool-card__identity"><PracticalToolIcon iconKey={tool.icon_key} /><span><strong>{tool.name}</strong>{tool.description && <small>{tool.description}</small>}</span></span><button aria-label={`${tool.name} 연결`} type="submit"><Plus size={16} /></button></form>) : <p>연결할 수 있는 도구가 없습니다.</p>}</div> : <form action={action} className="practical-tools-direct-form"><input name="scheduleId" type="hidden" value={item.id} /><label>이름 *<input name="name" required placeholder="예: 온라인 보건실" /></label><label>URL *<input name="url" required placeholder="https://" type="url" /></label><label>설명<input name="description" placeholder="간단한 설명" /></label><label>아이콘<select defaultValue="website" name="iconKey"><option value="online_health">온라인 보건실</option><option value="spreadsheet">Google Sheet</option><option value="drive">Google Drive</option><option value="school_system">학교 업무 시스템</option><option value="website">웹사이트</option><option value="other">기타</option></select></label>{state.message && <p className="form-message is-error" role="alert">{state.message}</p>}<button className="button button--primary" disabled={pending} type="submit">{pending ? "추가 중..." : "추가하고 연결"}</button></form>}</section>}
    </div>
...
export function PracticalScheduleWorkspace({ year, items, linkableEvents, linkedScheduleIds = [], newTitle, newMonth, newOpen = false, practicalTools = [], practicalToolsError = false, scheduleToolLinks = [], scheduleToolLinksError = false }: { readonly year: number; readonly items: readonly PracticalScheduleRow[]; readonly linkableEvents: readonly EventRow[]; readonly linkedScheduleIds?: readonly string[]; readonly newTitle?: string | undefined; readonly newMonth?: string | undefined; readonly newOpen?: boolean; readonly practicalTools?: readonly PracticalTool[]; readonly practicalToolsError?: boolean; readonly scheduleToolLinks?: readonly PracticalScheduleTool[]; readonly scheduleToolLinksError?: boolean }) {
  ...
  return <div className="practical-schedule-workspace">
    ...
    {toolsTarget && <ScheduleToolsModal item={toolsTarget} links={scheduleToolLinks} onClose={closeToolsModal} practicalToolsError={practicalToolsError} returnFocusRef={toolsTriggerRef} scheduleToolLinksError={scheduleToolLinksError} tools={practicalTools} />}
  </div>;
}
