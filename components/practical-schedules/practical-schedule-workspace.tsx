...
function ScheduleRow({ item }: { readonly item: PracticalScheduleRow }) {
  return <article className="practical-schedule-row">
    <div className="practical-schedule-row__main"><span className={`practical-category practical-category--${item.category}`}>{categoryLabels[item.category]}</span><strong>{item.title}</strong><small>{formatPracticalScheduleDate(item.scheduled_date)}{item.start_time ? ` · ${item.start_time.slice(0, 5)}~${item.end_time?.slice(0, 5) ?? ""}` : ""}</small></div>
...
export function PracticalScheduleWorkspace({ year, items, newTitle, newMonth }: { readonly year: number; readonly items: readonly PracticalScheduleRow[]; readonly newTitle?: string | undefined; readonly newMonth?: string | undefined }) {
  return <div className="practical-schedule-workspace">
    <div className="practical-schedule-toolbar"><div><strong>{year}학년도 실무 일정</strong><span>날짜가 확정된 업무는 캘린더와 Today에 자동으로 연결됩니다.</span></div><a className="button button--primary" href="#practical-schedule-add"><Plus size={16} />실무 일정 추가</a></div>
    <section className="practical-schedule-add" id="practical-schedule-add"><h2>{newTitle ? "연간 플래너 업무에서 추가" : "새 실무 일정"}</h2>{newTitle && newMonth && <p className="practical-schedule-add__context">{newMonth}월 추천 업무에서 가져왔습니다. 정확한 날짜는 확정 후 입력하세요.</p>}<ScheduleForm initial={newTitle ? { id: "", user_id: "", year, category: "staff", title: newTitle, scheduled_date: null, start_time: null, end_time: null, location: null, method: null, notes: null, url: null, annual_preset_key: null, created_at: "", updated_at: "" } : undefined} year={year} /></section>
    <section aria-label={`${year}년 실무 일정 목록`} className="practical-schedule-list"><div className="practical-schedule-list__header"><div><h2>연간 업무 상황판</h2><p>{items.length ? `${items.length}개 업무 · 날짜 미정 업무도 함께 관리합니다.` : "등록된 실무 일정이 없습니다."}</p></div><Link className="text-action" href={`/practical-schedules?year=${year}&new=1`}>새 항목 작성</Link></div>{items.length ? items.map((item) => <ScheduleRow item={item} key={item.id} />) : <div className="practical-schedule-empty"><p>실무 업무를 한 번 등록해보세요.</p><span>연간 플래너 추천 업무에서도 바로 추가할 수 있습니다.</span></div>}</section>
  </div>;
}
