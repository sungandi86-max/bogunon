  ...
  return (
    <form action={action} className="work-item-form" id={initialItem ? `edit-${initialItem.id}` : "create-work-item-form"}>
      ...
      {kind === "task" ? <div className="form-grid">
        ...
        <div className="field"><label className="field-label" htmlFor={`${formKey}-scheduled`}>수행일</label><input id={`${formKey}-scheduled`} name="scheduledDate" onChange={(e) => setScheduledDate(e.target.value)} required={Boolean(recurrence) || initialTemplate?.requiredDate === true} type="date" value={scheduledDate} />{recurrence && <small className="field-help">반복 생성의 기준일입니다.</small>}</div>
