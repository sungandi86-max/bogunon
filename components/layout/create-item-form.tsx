...
export function CreateItemForm({ defaultKind = "task", initialItem, initialTemplate, checklistItems = [], links = [], reminders = [], onSaved, titleRef }: CreateItemFormProps) {
  const mobileScheduleForm = initialTemplate?.key === "mobile-schedule" || initialTemplate?.key === "mobile-quick-memo";
  ...
  return (
    <form action={action} className="work-item-form" id={initialItem ? `edit-${initialItem.id}` : "create-work-item-form"}>
      ...
      {kind === "event" ? <>
        ...
        <div className="field"><label className="field-label" htmlFor={`${formKey}-project`}>프로젝트</label><select defaultValue={event?.project_id ?? initialTemplate?.projectId ?? ""} id={`${formKey}-project`} name="projectId"><option value="">프로젝트 없음</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select><small className="field-help">선택한 프로젝트의 상세 화면에 이 일정이 함께 표시됩니다.</small></div>
