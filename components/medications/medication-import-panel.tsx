...
export function MedicationImportPanel({ items, lots, today, onClose }: Props) {
  ...
  const [sheets, setSheets] = useState<Record<string, { readonly headers: readonly string[]; readonly rows: readonly MedicationImportRawRow[] }>>({});
  const [selectedSheet, setSelectedSheet] = useState("");
  useEffect(() => { if (state.status === "success") onClose(); }, [onClose, state.status]);

  function previewSheet(sheetName: string, parsedSheets: Record<string, { readonly headers: readonly string[]; readonly rows: readonly MedicationImportRawRow[] }> = sheets) {
    ...
    setPreview(buildMedicationImportPreview(parsed.rows, items, lots, today, parsed.headers));
  }
  ...
  async function readFile(file: File | undefined) {
    ...
    try {
      ...
      const parsedSheets = Object.fromEntries(sheetNames.map((sheetName) => {
        ...
        const rows = sheet ? XLSX.utils.sheet_to_json<MedicationImportRawRow>(sheet, { defval: "", raw: true }) : [];
