import { Bell, Circle, Copy, ExternalLink, ListChecks, Pencil, Repeat2, Save, Trash2 } from "lucide-react";
...
export function TaskList({ tasks, workflow, healthWorkflows }: { readonly tasks: readonly TaskRow[]; readonly workflow: WorkflowData; readonly healthWorkflows?: HealthWorkflowData | undefined }) {
  ...
  return <section className="work-item-list" aria-label="업무 목록">{tasks.map((task) => {
    ...
    <article className="work-item-card" key={task.id}>
      ...
      {sourceInstance && <a className="task-workflow-summary" href={`/workflows#workflow-${sourceInstance.id}`}><span><strong>Workflow 후속 업무</strong><small>{sourceInstance.name}에서 자동 생성됨</small></span><b>원본 보기</b></a>}
      ...
      <div className="work-item-actions"><details><summary><Pencil aria-hidden="true" size={16} />편집</summary><div className="inline-editor"><CreateItemForm checklistItems={checklist} initialItem={task} links={links} reminders={reminders} /></div></details><details><summary><Copy aria-hidden="true" size={16} />복제</summary><form action={duplicateWorkItemAction} className="duplicate-menu"><input name="id" type="hidden" value={task.id} /><input name="kind" type="hidden" value="task" /><label>새 날짜<input name="date" type="date" /></label><label><input defaultChecked name="includeChecklist" type="checkbox" />체크리스트</label><label><input defaultChecked name="includeDescription" type="checkbox" />설명</label><label><input defaultChecked name="includeMemo" type="checkbox" />메모</label>{task.recurrence_frequency && <label><input name="includeRecurrence" type="checkbox" />반복 설정</label>}<button className="button button--primary" type="submit">새 업무로 복제</button></form></details><form action={saveWorkItemAsTemplateAction}><input name="id" type="hidden" value={task.id} /><input name="kind" type="hidden" value="task" /><button className="icon-text-action" title="템플릿으로 저장" type="submit"><Save size={16} />템플릿</button></form><form action={deleteWorkItemAction}><input name="id" type="hidden" value={task.id} /><input name="kind" type="hidden" value="task" /><button className="danger-action" type="submit"><Trash2 aria-hidden="true" size={16} />삭제</button></form></div>
    </article>
  );})}</section>;
}
