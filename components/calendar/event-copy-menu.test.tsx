...
describe("EventCopyMenu", () => {
  it("offers same-day, next-day, next-week, and direct-date copy choices", () => {
    ...
    expect(screen.getByRole("button", { name: "같은 날짜에 복사" })).toHaveAttribute("value", "2026-07-22");
    expect(screen.getByRole("button", { name: "다음 날로 복사" })).toHaveAttribute("value", "2026-07-23");
    expect(screen.getByRole("button", { name: "다음 주 같은 요일로 복사" })).toHaveAttribute("value", "2026-07-29");
    expect(screen.getByLabelText("복사할 날짜 직접 선택")).toBeInTheDocument();
  });
  ...
  it("submits a directly selected date", () => {
    ...
    expect(screen.getByRole("button", { name: "선택한 날짜로 복사" })).toHaveAttribute("value", "2026-08-03");
  });
});
