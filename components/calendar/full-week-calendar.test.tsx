...
const schoolEvent: EventRow = { id: "event-2", user_id: "user", title: "교직원 회의", area: "schoolSchedule", start_date: "2026-07-18", end_date: "2026-07-18", is_all_day: true, start_time: null, end_time: null, memo: null, description: null, created_at: "", updated_at: "" };
...
describe("FullWeekCalendar", () => {
  ...
  it("renders a holiday with academic and health stickers in the weekly non-personal lane while preserving tasks and events", () => {
    render(<FullWeekCalendar date="2026-07-18" events={[event, schoolEvent]} onDropDate={vi.fn()} onMove={vi.fn()} onSelectDate={vi.fn()} selectedDate="2026-07-18" stickers={weekStickerRows} tasks={[task]} today="2026-07-18" />);

    expect(screen.getByRole("button", { name: "7월 18일 한글날 스티커 관리" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "7월 18일 병원 스티커 관리" })).toBeInTheDocument();
    expect(screen.getByText("+2")).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "7월 18일 AED 점검 스티커 관리" })).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "7월 18일 체육대회 스티커 관리" })).not.toBeInTheDocument();
    expect(screen.getAllByText("병원").length).toBeGreaterThan(0);
    expect(screen.getAllByText("교직원 회의").length).toBeGreaterThan(0);
    expect(screen.getAllByText("결과 보고").length).toBeGreaterThan(0);
  });
});
