...
export function EventList({ events, workflow }: { readonly events: EventRow[]; readonly workflow: WorkflowData }) {
  ...
  return <section className="calendar-list" aria-label="월간 일정 목록">{events.map((event) => {
    ...
    return <article className="calendar-list__item" key={event.id}><div><time>{event.start_date}{event.start_time ? ` ${event.start_time.slice(0, 5)}` : ""}</time><strong>{event.title}</strong>{links.map((link) => <a className="event-link" href={link.url} key={link.id} rel="noreferrer" target="_blank"><ExternalLink size={12} />{link.title}</a>)}{reminders.length > 0 && <small><Bell size={12} /> 알림 {reminders.length}</small>}</div><div className="work-item-actions"><details><summary><Pencil aria-hidden="true" size={16} />편집</summary><div className="inline-editor"><CreateItemForm initialItem={event} links={links} reminders={reminders} /></div></details><details><summary><Copy size={16} />복제</summary><form action={duplicateWorkItemAction} className="duplicate-menu"><input name="id" type="hidden" value={event.id} /><input name="kind" type="hidden" value="event" /><label>새 날짜<input name="date" type="date" /></label><label><input defaultChecked name="includeDescription" type="checkbox" />설명</label><label><input defaultChecked name="includeMemo" type="checkbox" />메모</label><button className="button button--primary" type="submit">새 일정으로 복제</button></form></details><form action={saveWorkItemAsTemplateAction}><input name="id" type="hidden" value={event.id} /><input name="kind" type="hidden" value="event" /><button className="icon-text-action" title="템플릿으로 저장" type="submit"><Save size={16} />템플릿</button></form><form action={deleteWorkItemAction}><input name="id" type="hidden" value={event.id} /><input name="kind" type="hidden" value="event" /><button className="danger-action" type="submit"><Trash2 aria-hidden="true" size={16} />삭제</button></form></div></article>;
  })}</section>;
}
