...
export function CalendarEntry({ compact = false, highlighted = false, item, kind, onMove }: { readonly compact?: boolean; readonly highlighted?: boolean; readonly item: EventRow | TaskRow; readonly kind: CalendarItemKind; readonly onMove?: ((value: MovableCalendarItem) => void) | undefined }) {
  ...
  return <div className={`calendar-item ${areaClass[item.area]}${highlighted ? " is-highlighted" : ""}${compact ? " is-compact" : ""}`} draggable={movable} onDragStart={(event) => {
    ...
    <span className="calendar-item__area">{areaLabel[item.area]}</span><span className="calendar-item__title">{item.title}</span>
