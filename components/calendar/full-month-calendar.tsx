...
export function FullMonthCalendar({ events = [], month = "2026-07", today = todayInSeoul() }: { readonly events?: EventRow[]; readonly month?: string; readonly today?: string }) {
        ...
        {Array.from({ length: 6 }, (_, weekIndex) => (
          <div className="full-calendar__row" key={weekIndex} role="row">
            {cells.slice(weekIndex * 7, weekIndex * 7 + 7).map((day, index) => {
              ...
              return <div aria-label={inMonth ? `${year}년 ${monthNumber}월 ${day}일, 일정 ${dayEvents.length}개` : "다른 달"} className={`full-calendar__cell${dayEvents.length ? " has-item" : ""}${date === today ? " is-today" : ""}`} key={`${day}-${index}`} role="gridcell">
