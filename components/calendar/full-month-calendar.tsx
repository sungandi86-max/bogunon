...
export function FullMonthCalendar({ events = [], highlight, month = "2026-07", onDropDate, onMove, onSelectDate, schoolStickers = [], selectedDate, tasks = [], today = todayInSeoul() }: Props) {
  ...
  return <section className="full-calendar" aria-label={`${year}년 ${monthNumber}월 월간 캘린더`} data-week-count={weekCount} role="grid">
    ...
    <div className="full-calendar__grid" role="rowgroup">{Array.from({ length: weekCount }, (_, weekIndex) => <div className="full-calendar__row" key={weekIndex} role="row">{cells.slice(weekIndex * 7, weekIndex * 7 + 7).map((cellDate, index) => {
      ...
      return <div aria-label={inMonth ? `${date}, 일정 ${dayEvents.length}개, 업무 ${dayTasks.length}개` : "다른 달"} className={`full-calendar__cell${date === today ? " is-today" : ""}${date === selectedDate ? " is-selected" : ""}${weekday === 0 ? " is-sunday" : weekday === 6 ? " is-saturday" : ""}`} key={`${date || "empty"}-${index}`} onDragOver={(event) => { if (date) event.preventDefault(); }} onDrop={(event) => { const raw = event.dataTransfer.getData("application/x-bogunon-calendar"); if (!raw || !date) return; try { const moved = JSON.parse(raw) as { id: string; kind: "event" | "task"; date: string }; onDropDate?.({ ...moved, newDate: date }); } catch { return; } }} role="gridcell">
        ...
        <div className={`calendar-date-stickers${daySchoolStickers.length > 0 ? " has-school" : ""}`}>{daySchoolStickers.slice(0, 1).map((item) => <StickerManagementButton date={date} key={item.id} label={item.label} recordId={item.id} recordType="calendar"><CalendarDateSticker compact highlighted={highlight === `sticker:${item.id}`} stickerKey={item.sticker_key} /></StickerManagementButton>)}{dayPersonalStickers.slice(0, 1).map((item) => <StickerManagementButton date={date} key={item.id} label={item.label} recordId={item.id} recordType="calendar"><CalendarDateSticker compact highlighted={highlight === `sticker:${item.id}`} stickerKey={item.sticker_key} /></StickerManagementButton>)}</div>
