...
import { buildTimeGridItems, layoutTimedItems, seoulDateTime, type AllDayGridItem, type PositionedTimedItem } from "@/lib/calendar/time-grid";
...
function TimedBlock({ item, onSelectItem }: { readonly item: PositionedTimedItem; readonly onSelectItem: Props["onSelectItem"] }) {
  ...
  return <button className={`time-calendar__event is-${item.colorKey}`} draggable={false} onClick={() => onSelectItem(item)} style={style} type="button"><strong>{item.title}</strong><span>{item.startTime}–{item.endTime}</span></button>;
}
