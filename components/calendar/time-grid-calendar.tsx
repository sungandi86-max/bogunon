...
import { moveCalendarEventTimeAction } from "@/app/(app)/calendar-event-actions";
...
function TimedBlock({ dragEnabled, item, onSelectItem }: { readonly dragEnabled: boolean; readonly item: PositionedTimedItem; readonly onSelectItem: Props["onSelectItem"] }) {
  ...
  return <button className={`time-calendar__event is-${item.colorKey}`} draggable={dragEnabled} onClick={() => onSelectItem(item)} onDragStart={dragEnabled ? (event) => event.dataTransfer.setData("application/x-bogunon-time-event", JSON.stringify(item)) : undefined} style={style} type="button"><strong>{item.title}</strong><span>{item.startTime}–{item.endTime}</span></button>;
}
...
export function TimeGridCalendar({ date, events, mode, onSelectDate, onSelectItem, onSelectSlot, selectedDate, stickers, tasks, today }: Props) {
  ...
  const dragEnabled = useDesktopDragEnabled();
  ...
  return <section aria-label={mode === "week" ? "주간 시간표" : "일간 시간표"} className={`time-calendar is-${mode}`}>
    ...
    <div className="time-calendar__scroll"><div className="time-calendar__hours">{hours.map((hour) => <time dateTime={`${String(hour).padStart(2, "0")}:00`} key={hour}>{String(hour).padStart(2, "0")}:00</time>)}</div><div className="time-calendar__days" style={{ gridTemplateColumns: `repeat(${dates.length}, minmax(0, 1fr))` }}>{dates.map((day) => <div className={`time-calendar__day${day === today ? " is-today" : ""}`} key={day}>{hours.slice(0, -1).map((hour) => <button aria-label={`${day} ${String(hour).padStart(2, "0")}:00 일정 추가`} className="time-calendar__slot" key={hour} onClick={() => onSelectSlot(day, hour * 60)} onDragOver={dragEnabled ? (event) => event.preventDefault() : undefined} onDrop={dragEnabled ? (event) => { event.preventDefault(); dropTimedItem(day, hour * 60, event.dataTransfer.getData("application/x-bogunon-time-event")); } : undefined} type="button" />)}{positioned.filter((item) => item.date === day).map((item) => <TimedBlock dragEnabled={dragEnabled} item={item} key={item.id} onSelectItem={onSelectItem} />)}{day === now.date && now.minute >= HOUR_START * 60 && now.minute <= HOUR_END * 60 && <div aria-label={`현재 시간 ${Math.floor(now.minute / 60)}:${String(now.minute % 60).padStart(2, "0")}`} className="time-calendar__now" style={{ top: ((now.minute - HOUR_START * 60) / 60) * HOUR_HEIGHT }}><span /></div>}</div>)}</div></div>
  </section>;
}
