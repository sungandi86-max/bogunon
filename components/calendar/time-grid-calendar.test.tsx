...
vi.mock("@/app/(app)/calendar-event-actions", () => ({ moveCalendarEventTimeAction: vi.fn(async () => ({ status: "success" })) }));
...
describe("TimeGridCalendar", () => {
  ...
  it("does not expose timed event dragging on mobile", () => {
    vi.stubGlobal("matchMedia", vi.fn(() => ({
      matches: false,
      addEventListener: vi.fn(),
      removeEventListener: vi.fn(),
    ...
    render(<TimeGridCalendar date="2026-07-22" events={[event]} mode="day" onSelectDate={vi.fn()} onSelectItem={vi.fn()} onSelectSlot={vi.fn()} selectedDate="2026-07-22" stickers={[]} tasks={[]} today="2026-07-22" />);

    expect(screen.getByRole("button", { name: /기말고사/ })).toHaveAttribute("draggable", "false");
  });
});
