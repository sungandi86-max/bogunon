...
type CalendarFilter = "all" | "work" | "school" | "personal" | "stickers" | "schoolStickers" | "personalStickers";
const options: ReadonlyArray<[CalendarFilter, string]> = [["all","전체"],["work","업무"],["school","학교"],["personal","개인"],["stickers","날짜 스티커 전체"],["schoolStickers","학교 스티커"],["personalStickers","개인 스티커"]];
...
export function CalendarWorkspace({ events, highlight, initialDate, initialStickerOpen = false, initialView, stickers, tasks, today, toolbarAction, workflow }: Props) {
  ...
  return <>
    ...
    {initialView === "month" ? <FullMonthCalendar events={showEntries ? periodEvents : []} highlight={highlight} month={selectedDate.slice(0, 7)} onDropDate={openDropMove} onMove={(value) => setMoveState({ value })} onSelectDate={setSelectedDate} schoolStickers={visibleStickers} selectedDate={selectedDate} tasks={showEntries ? periodTasks : []} today={today} /> : <FullWeekCalendar date={selectedDate} events={showEntries ? periodEvents : []} highlight={highlight} onDropDate={openDropMove} onMove={(value) => setMoveState({ value })} onSelectDate={setSelectedDate} selectedDate={selectedDate} stickers={visibleStickers} tasks={showEntries ? periodTasks : []} today={today} />}
