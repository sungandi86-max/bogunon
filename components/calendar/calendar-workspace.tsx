...
type CalendarFilter = "all" | "work" | "school" | "personal" | "exercise" | "stickers" | "schoolStickers" | "personalStickers";
const options: ReadonlyArray<[CalendarFilter, string]> = [["all","전체"],["work","업무"],["school","학교"],["personal","개인"],["exercise","운동"],["stickers","날짜 스티커 전체"],["schoolStickers","학교 스티커"],["personalStickers","개인 스티커"]];
...
export function CalendarWorkspace({ events, exerciseLogs, exerciseStickers, highlight, initialDate, initialStickerOpen = false, initialView, stickers, tasks, today, toolbarAction, workflow }: Props) {
  ...
  const showEntries = !["exercise", "stickers", "schoolStickers", "personalStickers"].includes(filter);
  ...
  return <>
    ...
    {initialView === "month" ? <FullMonthCalendar events={showEntries ? periodEvents : []} exerciseLogs={showExercise ? exerciseLogs : []} exerciseStickers={exerciseStickers} highlight={highlight} month={selectedDate.slice(0, 7)} onDropDate={openDropMove} onMove={(value) => setMoveState({ value })} onSelectDate={setSelectedDate} schoolStickers={visibleStickers} selectedDate={selectedDate} tasks={showEntries ? periodTasks : []} today={today} /> : <FullWeekCalendar date={selectedDate} events={showEntries ? periodEvents : []} exerciseLogs={showExercise ? exerciseLogs : []} exerciseStickers={exerciseStickers} highlight={highlight} onDropDate={openDropMove} onMove={(value) => setMoveState({ value })} onSelectDate={setSelectedDate} selectedDate={selectedDate} stickers={visibleStickers} tasks={showEntries ? periodTasks : []} today={today} />}
