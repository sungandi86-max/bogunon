...
const packs = [["school", "학교"], ["academic", "학사일정"], ["personal", "개인"]] as const satisfies readonly (readonly [CalendarStickerPack, string])[];
...
export function SchoolStickerPicker({ stickers, today }: { readonly stickers: readonly CalendarStickerRow[]; readonly today: string }) {
  ...
  const visibleCatalog = useMemo(() => filterCalendarStickers(catalog, query, pack === "academic" ? group : "all"), [catalog, group, pack, query]);
  ...
  return <section className="school-sticker-picker" aria-labelledby="calendar-sticker-title">
    ...
    <div aria-label="날짜 스티커 팩" className="calendar-sticker-tabs" role="tablist">
      {packs.map(([value, label]) => <button aria-controls="calendar-sticker-panel" aria-selected={pack === value} id={`calendar-sticker-${value}-tab`} key={value} onClick={() => selectPack(value)} role="tab" type="button">{label}</button>)}
    </div>
    <form action={attachAction} aria-labelledby={`calendar-sticker-${pack}-tab`} className="school-sticker-picker__form" id="calendar-sticker-panel" role="tabpanel">
      ...
      {pack === "academic" && <div aria-label="학사일정 카테고리" className="calendar-sticker-groups" role="group">{academicGroups.map(([value, label]) => <button aria-pressed={group === value} key={value} onClick={() => { setGroup(value); setSelectedKey(null); }} type="button">{label}</button>)}</div>}
