...
export function SchoolInfoCard({ school }: { readonly school: UserSchoolSettings | null }) {
  const schoolSummary = school ? `${school.name}${school.region ? ` · ${school.region}` : ""}` : "학교 미설정";

  return <details className="rail-module school-info-card">
    <summary><span><School aria-hidden="true" size={17} /><span><strong>학교 정보</strong><small title={schoolSummary}>{schoolSummary}</small></span></span></summary>
