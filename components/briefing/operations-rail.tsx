...
import { CalendarPlus, CheckSquare2, MessageSquareMore, Plus } from "lucide-react";
...
export function OperationsRail({ eventsToday, priorityTasks, waitingTasks }: { readonly eventsToday: EventRow[]; readonly priorityTasks: TaskRow[]; readonly waitingTasks: TaskRow[] }) {
  ...
  return <aside className="operations-rail" aria-label="오늘의 운영 항목">
    ...
    <section className="rail-module" aria-labelledby="today-work-title"><div className="rail-heading"><h2 id="today-work-title">오늘 우선 업무</h2><strong>{priorityTasks.length}</strong></div>{priorityTasks.length ? priorityTasks.slice(0, 4).map((task) => <div className="rail-row" key={task.id}><strong>{task.title}</strong><Badge tone="deadline">우선</Badge></div>) : <p className="rail-empty">우선 업무가 없습니다.</p>}</section>
