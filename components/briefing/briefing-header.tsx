...
export function BriefingHeader({ priorityCount, nextEvent, today }: { readonly priorityCount: number; readonly nextEvent: EventRow | undefined; readonly today: string }) {
  ...
  return <header className="briefing-header"><div><div className="mobile-briefing-brand"><span className="mobile-briefing-brand__mark" aria-hidden="true">온</span><strong>보건온</strong><small>오늘</small></div><h1 className="briefing-header__date"><span>{dateLabel.format(date)}</span><time className="briefing-header__time" suppressHydrationWarning>{timeLabel.format(new Date())}</time></h1><p className="mobile-briefing-greeting">오늘도 차분하게 업무를 정리해보세요.</p><p className="briefing-header__summary">{summary}</p></div><div className="work-status" aria-label="저장 상태"><span><ShieldCheck aria-hidden="true" size={17} />저장 완료</span><strong><Clock3 aria-hidden="true" size={15} />동기화 완료</strong></div></header>;
}
