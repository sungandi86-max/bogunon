...
export async function MealCard({ date, school }: { readonly date: string; readonly school: NeisDefaultSchool | null }) {
  if (!school) return <section className="rail-module school-daily-card" aria-labelledby="meal-title"><div className="rail-heading"><h2 id="meal-title">오늘의 급식</h2><Utensils aria-hidden="true" size={17} /></div><p className="rail-empty">학교를 설정하면 급식을 확인할 수 있습니다.</p><Link className="rail-card-link" href="/settings">학교 설정</Link></section>;
  ...
  return <section className="rail-module school-daily-card" aria-labelledby="meal-title">
    <div className="rail-heading"><div><span className="rail-kicker">{date.replaceAll("-", ". ")}</span><h2 id="meal-title">오늘의 급식</h2></div><Utensils aria-hidden="true" size={17} /></div>
