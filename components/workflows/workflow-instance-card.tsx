...
export function WorkflowInstanceCard({ data, instance, task }: { readonly data: HealthWorkflowData; readonly instance: TaskWorkflowInstanceRow; readonly task?: TaskRow | undefined }) {
  ...
  return <article className="workflow-instance-card" id={`workflow-${instance.id}`}>
    ...
    <div className="workflow-instance-actions">{instance.status === "active" && <><form action={transitionWorkflowAction}><input name="instanceId" type="hidden" value={instance.id} /><input name="status" type="hidden" value="paused" /><WorkflowSubmitButton className="button button--ghost">일시 중지</WorkflowSubmitButton></form><form action={transitionWorkflowAction}><input name="instanceId" type="hidden" value={instance.id} /><input name="status" type="hidden" value="completed" /><WorkflowSubmitButton className="button button--secondary" confirmMessage="모든 단계를 확인하고 업무 절차를 완료할까요?">업무 절차 완료</WorkflowSubmitButton></form></>}{instance.status === "paused" && <form action={transitionWorkflowAction}><input name="instanceId" type="hidden" value={instance.id} /><input name="status" type="hidden" value="active" /><WorkflowSubmitButton className="button button--secondary">재개</WorkflowSubmitButton></form>}{(instance.status === "active" || instance.status === "paused") && <form action={transitionWorkflowAction}><input name="instanceId" type="hidden" value={instance.id} /><input name="status" type="hidden" value="cancelled" /><WorkflowSubmitButton className="danger-action" confirmMessage="이 업무 절차를 취소할까요? 취소 후에는 다시 열 수 없습니다.">취소</WorkflowSubmitButton></form>}<AssistantTrigger entityId={instance.id} label="단계 추천" surface="workflow" /></div>
