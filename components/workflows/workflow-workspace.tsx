...
export function WorkflowWorkspace({ data, tasks }: { readonly data: HealthWorkflowData; readonly tasks: readonly TaskRow[] }) {
  ...
  return <div className="workflow-layout"><section className="workflow-main"><div className="workflow-tools"><label className="task-search"><span className="sr-only">Workflow 검색</span><Search size={18} /><input onChange={(event) => setQuery(event.target.value)} placeholder="Workflow, 업무, 단계, 메모 검색" type="search" value={query} /></label><div className="workflow-filters"><select aria-label="상태" onChange={(event) => setStatus(event.target.value)} value={status}><option value="all">모든 상태</option><option value="active">진행 중</option><option value="paused">일시 중지</option><option value="completed">완료</option><option value="cancelled">취소</option></select><select aria-label="카테고리" onChange={(event) => setCategory(event.target.value)} value={category}><option value="all">모든 카테고리</option>{TASK_CATEGORY_OPTIONS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select><select aria-label="진행률" onChange={(event) => setProgress(event.target.value)} value={progress}><option value="all">모든 진행률</option><option value="low">50% 미만</option><option value="high">50% 이상</option></select><label className="workflow-blocked-filter"><input checked={blocked} onChange={(event) => setBlocked(event.target.checked)} type="checkbox" />보류 단계만</label>{(query || status !== "all" || category !== "all" || progress !== "all" || blocked) && <button className="filter-reset" onClick={() => { setQuery(""); setStatus("all"); setCategory("all"); setProgress("all"); setBlocked(false); }} type="button"><X size={14} />초기화</button>}</div></div><p className="task-results-summary">Workflow {visible.length}건</p>{visible.length ? <div className="workflow-instance-list">{visible.map((instance) => <WorkflowInstanceCard data={data} instance={instance} key={instance.id} task={tasks.find((task) => task.id === instance.task_id)} />)}</div> : <section className="empty-state"><div><h2>조건에 맞는 Workflow가 없습니다.</h2><p>업무에 기본 또는 내 템플릿을 적용해 시작하세요.</p></div></section>}</section><WorkflowTemplatePanel data={data} tasks={tasks} /></div>;
}
