...
export async function markNoticeReadAction(formData: FormData): Promise<void> { await markNoticeRead(z.string().uuid().parse(formData.get("id"))); revalidatePath("/", "layout"); revalidatePath("/notices"); }
