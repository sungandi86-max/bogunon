...
export default async function ProtectedLayout({ children }: ProtectedLayoutProps) {
  ...
  const [presetPreferences, currentProfile, notices, projects] = await Promise.all([
    listHealthPresetPreferences(),
    getCurrentProfile(),
    listNotices(),
    listProjects(),
  ]);

  return <AppShell notices={notices} presetPreferences={presetPreferences} profile={createAuthProfile(currentProfile.email, { displayName: currentProfile.displayName, avatarUrl: currentProfile.avatarUrl, role: currentProfile.role })} projects={projects}>{children}</AppShell>;
}
