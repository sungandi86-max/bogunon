...
describe("saveWorkItemAction", () => {
  ...
  it("stores a workout as a calendar plan with structured details", async () => {
    const formData = new FormData();
    formData.set("kind", "event");
    formData.set("title", "필라테스 수업");
    formData.set("area", "exercise");
    formData.set("eventType", "workout");
    formData.set("workoutType", "필라테스");
    formData.set("startDate", "2026-07-25");
    formData.set("endDate", "2026-07-25");
    formData.set("isAllDay", "on");
    ...
    expect(vi.mocked(saveEventBundle)).toHaveBeenCalledWith(expect.objectContaining({
      area: "exercise",
      event_type: "workout",
      event_details: { kind: "workout", workoutType: "필라테스" },
    }), { links: [], reminders: [] }, undefined);
  });

  it("requires a tournament name before saving tournament details", async () => {
    const formData = new FormData();
    formData.set("kind", "event");
    formData.set("title", "주말 대회");
    formData.set("area", "exercise");
