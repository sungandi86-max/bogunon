...
export async function moveCalendarEventTimeAction(
  formData: FormData,
): Promise<CalendarEventActionState> {
  ...
  if (!id
    ...
    || endTime <= startTime) {
    return { status: "error", message: "변경할 일정 시간을 확인해 주세요." };
  }
  ...
  try {
    ...
    return {
      status: "error",
      message: error instanceof Error ? error.message : "일정 시간을 변경하지 못했습니다.",
    };
  }
}
