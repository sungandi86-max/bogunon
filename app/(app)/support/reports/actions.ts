...
export async function createReportAction(_state: ReportActionState, formData: FormData): Promise<ReportActionState> {
  ...
  try {
    ...
    await createReport({ reportType, title, description, attemptedAction: reportType === "bug" ? optional(value(formData, "attemptedAction")) : null, observedResult: reportType === "bug" ? optional(value(formData, "observedResult")) : null, reproducible: reportType === "bug" && isReproducible(reproducibleValue) ? reproducibleValue : null, pagePath: value(formData, "pagePath"), appVersion: optional(value(formData, "appVersion")), userAgent: optional(value(formData, "userAgent")), viewportWidth: numberValue(formData, "viewportWidth"), viewportHeight: numberValue(formData, "viewportHeight") });
...
export async function updateReportAction(formData: FormData): Promise<void> {
  ...
  await updateReport(id, status, optional(value(formData, "adminNote")));
