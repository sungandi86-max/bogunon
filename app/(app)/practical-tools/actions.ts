...
function parse(formData: FormData): { name: string; description: string | null; url: string; icon_key: PracticalToolIconKey; scope: "public" | "personal"; is_active: boolean } | string {
  ...
  const scope = value(formData, "scope") === "public" ? "public" : "personal";
  if (url.startsWith("/") && !url.startsWith("//")) return { name, description: value(formData, "description") || null, url, icon_key: icons.includes(icon) ? icon : "other", scope, is_active: formData.has("isActive") };
  try { if (!/^https?:$/i.test(new URL(url).protocol)) return "http 또는 https 주소만 사용할 수 있습니다."; } catch { return "URL을 확인해 주세요."; }
  return { name, description: value(formData, "description") || null, url, icon_key: icons.includes(icon) ? icon : "other", scope, is_active: formData.has("isActive") };
}
...
export async function savePracticalToolAction(_state: PracticalToolActionState, formData: FormData): Promise<PracticalToolActionState> {
  ...
  try {
    ...
    const scopeError = validatePracticalToolScope(parsed.scope, parsed.icon_key, isAdminRole(profile.role));
    if (scopeError) return { status: "error", message: scopeError };
    const id = value(formData, "id");
    if (id) await updatePracticalTool(id, { name: parsed.name, description: parsed.description, url: parsed.url, icon_key: parsed.icon_key, is_active: parsed.is_active });
