...
export default async function TasksPage() {
  ...
  const [tasks, events, workflow] = await Promise.all([listTasks(), listAllEvents(), listWorkflowData()]);
  return <main className="page-canvas"><PageHeader action={<MobileCreateButton />} description="업무를 검색하고 카테고리, 반복 주기와 상태를 관리합니다." title="업무" /><TaskWorkspace events={events} tasks={tasks} today={today} workflow={workflow} /></main>;
}
