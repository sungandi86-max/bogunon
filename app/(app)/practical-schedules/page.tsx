...
export default async function PracticalSchedulesPage({ searchParams }: { readonly searchParams: Promise<{ year?: string; new?: string; title?: string; month?: string }> }) {
  ...
  const [items, linkableEvents, linkedScheduleIds, tools] = await Promise.all([listPracticalSchedules(year), listLinkableEvents(year), listLinkedPracticalScheduleIds(), listPracticalTools().catch(() => [])]);
